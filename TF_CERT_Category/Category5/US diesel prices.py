...
from tensorflow.keras.layers import Dense,GlobalAveragePooling2D,Bidirectional,LSTM,Dropout
...
def solution_model():
    ...
    N_FEATURES = len(df.columns) # DO NOT CHANGE THIS -> 1

    # Normalizes the data
    data = df.values
    data = normalize_series(data, data.min(axis=0), data.max(axis=0)) # 쉽게말해서 minmax scaler해준다.

    # Splits the data into training and validation sets.
    SPLIT_TIME = int(len(data) * 0.8) # DO NOT CHANGE THIS  # 1424 * 0.8 + int형
    ...
    model = tf.keras.models.Sequential([
        ...
        Bidirectional(tf.keras.layers.LSTM(BATCH_SIZE, return_sequences=True, input_shape=(N_PAST, N_FEATURES))),
        tf.keras.layers.Dense(BATCH_SIZE, activation='relu'),
        Bidirectional(tf.keras.layers.LSTM(128, return_sequences=True, input_shape=(N_FUTURE, N_FEATURES))),
    ...
    optimizer = tf.keras.optimizers.Adam(lr=1e-3) # YOUR CODE HERE
    es = tf.keras.callbacks.EarlyStopping(monitor='val_loss', mode='min', patience=10, restore_best_weights=True)
    lr = tf.keras.callbacks.ReduceLROnPlateau(monitor='val_loss', mode='min', patience=5, factor=0.5)
    model.compile(
        # YOUR CODE HERE
