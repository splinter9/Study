...
#2. 모델구성
model = Sequential()
model.add(Dense(355, input_dim=13))
...
model.fit(x_train, y_train, epochs=10000, validation_split=0.5, verbose=1, batch_size=1, callbacks=[mcp,es])
...
result_recover = np.argmax(results, axis=1)
results = results.round(0).astype(int)

submit_file['target'] = results
submit_file.to_csv(path+f"heart_results.csv", index = False)
