...
x1=sam.drop(range(893,1120), axis=0)
x2=kim.drop(range(893,1060), axis=0)


x1=x1.loc[::-1].reset_index(drop=True)
x2=x2.loc[::-1].reset_index(drop=True)

x1 = x1.drop(columns=['일자','시가', "Unnamed: 6", '전일비' , '등락률','금액(백만)', '신용비' , '개인', '기관','외인(수량)', '외국계' , '프로그램' , '외인비','거래량'], axis=1)
x2 = x2.drop(columns=['일자','시가', "Unnamed: 6", '전일비' , '등락률','금액(백만)', '신용비' , '개인', '기관','외인(수량)', '외국계' , '프로그램' , '외인비', '거래량'], axis=1)
x1 = np.array(x1)
x2 = np.array(x2)
...
dense1=LSTM(50, activation='relu', name='dense1')(input1)
...
dense11=LSTM(50, activation='relu', name='dense11')(input2)
...
model.save("D:\\_data\\exam\\삼성키움_시가.h5")
